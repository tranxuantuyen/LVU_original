...
def soft_label_loss_softmax(preds, soft_labels, temperature):
    ...
    distillation_loss = F.kl_div(
        soft_log_probs, soft_targets.detach(), reduction="batchmean"
    )
    return distillation_loss * temperature**2.0
    ...
    def __init__(self, config, args):
        ...
        if args.mask_sep:
            self.masked_embedding = [
                nn.Parameter(torch.zeros((cur_dim,))) for cur_dim in args.all_feat_dims
            ]
                ...
                if args.train_long_term:
                    ...
                    if args.train_long_term_linear:
                        self.long_term_head = RobertaLinearHead(
                            config, in_dim=in_dim, out_size=args.num_long_term_classes
                        )
                    else:
                ...
                if args.mask_sep:
                    if args.train_long_term:
                        ...
                        if args.num_long_term_classes == -1:
                            all_loss["lm_action"] = (
                                (scores[:, 0] - long_term_labels[:, 1]) ** 2.0
                            ).mean()
                        else:
                            all_loss["lm_action"] = CrossEntropyLoss()(
                                scores, long_term_labels[:, 1]
                            )
                    else:
                        ...
                        if args.same_movie:
                            ...
                            all_loss["same_movie"] = (
                                same_movie_loss * args.same_movie_weight
                            )
